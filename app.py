...
st.markdown("""
    ...
    /* ---------------------------------------------------------
       SMOOTH HOVER GLOW CARDS & EXPANDERS
       --------------------------------------------------------- */
...
try:
    ...
    if response.status_code == 200:
        st.sidebar.markdown('<p><span class="live-indicator"></span>Backend: Operational</p>', unsafe_allow_html=True)
    else:
...
with tab1:
    ...
    if st.button("Initialize Global Research", type="primary"):
        if query:
            ...
            # Sleek enterprise synthesis card replacing the default green success box
            st.markdown(f"""
                <div style="background-color: #111827; border: 1px solid #0284c7; border-radius: 8px; padding: 16px; margin: 16px 0;">
                    <div style="color: #38bdf8; font-weight: 600; font-size: 0.95rem; margin-bottom: 6px;">SYNTHESIZED GRAPH INTELLIGENCE REPORT</div>
            ...
            st.caption("Tap any entity card below to inspect its full structural connection and verification source.")
            
            for item in lineage_data:
                with st.expander(f"🏢 {item['Entity']} — [{item['Connection Type']}]"):
                    col_det1, col_det2 = st.columns(2)
                    col_det1.markdown(f"**Source Origin:** `{item['Source']}`")
                    col_det2.markdown(f"**Confidence / State:** `{item['Confidence']}` ({item['Status']})")

            # Hidden Details Drawer for Cypher/Payload
            with st.expander("💻 FastMCP Tool Execution Payload & Generated Cypher"):
                st.code(f"""
// Executed Cypher Query against Neo4j Engine for {target_clean}
MATCH path = (e:Entity)-[*1..{max_depth}]-(connected)
WHERE e.workspace = '{active_workspace}'
  AND (e.name CONTAINS '{target_clean}' OR connected.description CONTAINS '{target_clean}')
