...
with tab3:
    ...
    # Inspector Drawer for Selected Entities ("Details hidden inside, accessible on demand")
    with st.expander("🔍 Deep Node Attribute & Entity Metadata Inspector", expanded=False):
        ...
        with col_sel2:
            ...
            st.blockquote("""
            "...In the event that uptime falls below 99.9%, a 15% financial penalty shall be applied 
            to the invoice total for the subsequent billing cycle under Agreement 2026..."
            """)
...
async def query_graph():
    async with Client("{BACKEND_URL}/sse") as client:
        # Traverses Knowledge Graph via FastMCP
