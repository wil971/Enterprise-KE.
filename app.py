...
with tab1:
    ...
    # Professional enterprise terminology update
    ...
    if st.button("Run GraphRAG Search", type="primary"):
        if query:
            ...
            st.session_state["last_answer"] = f"Based on enterprise knowledge graph analysis, active contract terms, SLA obligations, and vendor entity relationships matched: '{query}'."

            st.markdown("### 🤖 Synthesized Graph Response")
            st.success(st.session_state["last_answer"])
            
            # Feedback & RLHF Loop for enterprise tuning
            ...
            # Render interactive lineage with confidence bars & source text preview drawers
            for idx, row in enumerate(lineage_data):
            ...
            # Compliance Audit Export Package Button
...
with tab3:
    ...
    with st.expander("🔍 Deep Node Attribute & Entity Metadata Inspector", expanded=False):
        ...
        with col_sel2:
            ...
            st.markdown("> *...In the event that uptime falls below 99.9%, a 15% financial penalty shall be applied...*")
...
with tab4:
    ...
    st.caption("Connect external software applications, custom agents, or data microservices.")
