...
with tab1:
    ...
    st.caption("Query your internal knowledge graph or activate deep-web and corporate API agents for real-world entity resolution.")
    
    # Global Intelligence Target Input
    query = st.text_input(
        "Enter company name, vendor, or intelligence target:",
        placeholder="e.g., Investigate Acme Corp, its subsidiaries, and global risk exposure"
    )
    ...
    with col_src2:
        use_web = st.checkbox("🌍 Live Web & News Agents", value=True, help="Deploy FastMCP agents to search live web sentiment & news.")
    with col_src3:
        use_api = st.checkbox("🏢 Corporate APIs (SEC / OpenCorp)", value=True, help="Pull live corporate hierarchies and ownership.")

    if st.button("Initialize Deep Research", type="primary"):
        if query:
            # Dynamic Agentic Loading Simulation
            with st.status("Executing Deep Intelligence Gathering...", expanded=True) as status:
                if use_internal:
                    st.write("🔗 Querying internal Neo4j knowledge mesh...")
                    time.sleep(0.4)
                if use_web:
                    st.write("🌍 Dispatching web-agents to gather live risk sentiment...")
                    time.sleep(0.6)
                if use_api:
                    st.write("🧬 Resolving external corporate hierarchies and board members...")
                    time.sleep(0.5)
                st.write(f"🧠 Synthesizing multi-source data via {search_mode}...")
                time.sleep(0.4)
                status.update(label="Entity Resolution Complete!", state="complete", expanded=False)
            
            st.markdown("### 🤖 Synthesized Intelligence Brief")
            st.success(f"**Target:** {query}\n\n**Global Synthesis:** Successfully merged internal graph metrics with live external corporate registries and web sentiment. Identified key cross-entity structures and risk vectors matching the target.")
            
            with st.expander("🔍 Deep Graph & Multi-Source Resolution Path", expanded=True):
                st.markdown("**Cross-Entity Linkage & Reasoning:**")
                if use_internal:
                    st.write("1. **Internal Graph Mesh:** Found matching active contracts, SLAs, and vendor nodes.")
                if use_api:
                    st.write("2. **Corporate Registry API:** Linked target entity to parent holdings and verified board structure.")
                if use_web:
                    st.write("3. **Live Web Scraper Agent:** Detected recent regulatory filings and news mentions.")
                st.write(f"4. **FastMCP Engine ({search_mode}, Depth {max_depth}):** Unified all data points into a synchronized workspace context.")
            
            st.markdown("### 🌐 Consolidated Entity Lineage")
            st.caption("Verifiable audit trail connecting insights directly to graph nodes, APIs, and web records.")
            
            lineage_data = [
                {"Entity": f"{query} (Target)", "Connection Type": "Primary Entity", "Source": "Global Knowledge Mesh", "Confidence": "99.1%", "Status": "Synced"},
                {"Entity": "Global Holdings LLC", "Connection Type": "Parent Corporation", "Source": "Corporate Registry API", "Confidence": "98.2%", "Status": "Verified"},
                {"Entity": "Executive Leadership", "Connection Type": "Key Personnel", "Source": "Live Web Search Agent", "Confidence": "91.5%", "Status": "Live"}
            ]
            ...
            with st.expander("💻 FastMCP Tool Execution Payload & Generated Cypher"):
                st.code(f"""
// Executed Payload & Global Traversal Query via FastMCP ({search_mode})
{{
    "target_query": "{query}",
