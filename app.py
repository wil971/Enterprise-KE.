...
with tab1:
    ...
    if st.button("Run GraphRAG Search", type="primary"):
        if query:
            ...
            st.success(f"**Answer:** Based on knowledge graph analysis, active contract terms, SLA obligations, and vendor relationships matched: *'{query}'*.")
...
with tab3:
    ...
    with st.expander("🔍 Deep Node Attribute & Entity Metadata Inspector", expanded=False):
        ...
        with col_sel2:
            ...
            st.markdown("> *...Active telemetry indicates live packet synchronization across the multi-hop network nodes...*")
